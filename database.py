...
EMPLOYEE_EXCEL = r"c:\Users\Gonzalo Valdivia\Documents\ERP REMU\listado_empleados.xlsx"
...
def load_employees_from_excel():
    ...
    for path, label in paths_to_load:
        ...
        for row in sheet.iter_rows(min_row=3, values_only=True):
            if not row or not row[0]: # Skip if RUT is empty
                continue
            ...
            rut = clean_rut(row[0])
            contrato = int(row[56]) if len(row) > 56 and row[56] is not None else 1
            nombre = row[1] if len(row) > 1 else ""
            sexo = row[2] if len(row) > 2 else ""
            fecha_nac = format_date(row[3]) if len(row) > 3 else ""
            est_civil = row[4] if len(row) > 4 else ""
            comuna = row[7] if len(row) > 7 else ""
            correo = row[14] if len(row) > 14 else ""
            telefono = row[13] if len(row) > 13 else ""
            banco = row[15] if len(row) > 15 else ""
            cuenta_banco = str(row[16]) if len(row) > 16 and row[16] is not None else ""
            forma_pago = row[18] if len(row) > 18 else ""
            afp = str(row[20]).lower() if len(row) > 20 and row[20] is not None else ""
            cotiz_afp = row[21] if len(row) > 21 else 0.0
            isapre = str(row[26]).lower() if len(row) > 26 and row[26] is not None else ""
            moneda_isapre = row[27] if len(row) > 27 else ""
            cotiz_uf = row[28] if len(row) > 28 and row[28] is not None else 0.0
            cotiz_pesos = row[29] if len(row) > 29 and row[29] is not None else 0.0
            tramo_asig = row[35] if len(row) > 35 else "D"
            tipo_contrato = row[60] if len(row) > 60 else ""
            f_inicio = format_date(row[62]) if len(row) > 62 else ""
            f_termino = format_date(row[63]) if len(row) > 63 else ""
            sueldo_base = int(row[66]) if len(row) > 66 and row[66] is not None else 0
            cargo = str(row[68]).strip().upper() if len(row) > 68 and row[68] is not None else ""
            cc = row[72] if len(row) > 72 else ""
            sede = row[73] if len(row) > 73 else ""
            hrs_semanales = row[82] if len(row) > 82 and row[82] is not None else 40.0
            
            afecto_afc = 1 if len(row) > 84 and (row[84] is True or str(row[84]).lower() in ("true", "1", "s", "si")) else 0
            num_hijos = int(row[163]) if len(row) > 163 and row[163] is not None else 0
            ...
            INSERT OR REPLACE INTO empleados (
                ...
                centro_costo, sede, horas_semanales, afecto_seguro_cesantia, numero_hijos, raw_json
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ...
                cc, sede, hrs_semanales, afecto_afc, num_hijos, raw_json
            ))
            count += 1
...
def load_cargos_mapping():
    ...
    if not os.path.exists("CARGOS.xlsx"):
        print("Warning: CARGOS.xlsx not found!")
        return

    wb = openpyxl.load_workbook("CARGOS.xlsx", data_only=True)
    sheet = wb.active
...
def load_proyectos_mapping():
    ...
    if not os.path.exists("Proyectos.xlsx"):
        print("Warning: Proyectos.xlsx not found!")
        return

    wb = openpyxl.load_workbook("Proyectos.xlsx", data_only=True)
    sheet = wb.active
...
def load_all_justifications():
    ...
    # 1. Load May (2026-05)
    may_extra = {}
    if os.path.exists("PLANILLA GENERAL MAYO 2026.xlsx"):
        try:
            wb = openpyxl.load_workbook("PLANILLA GENERAL MAYO 2026.xlsx", data_only=True)
            sheet = wb["Detalle"]
    ...
    hist_periods = {
        '2026-01': (r'Datos de entrada\PLANILLA GENERAL ENERO 2026.xlsx', 'Planilla General Enero 2026'),
        '2026-02': (r'Datos de entrada\PLANILLA GENERAL FEBRERO 2026.xlsx', 'PLANILLA GENERAL FEBRERO 2026'),
        '2026-03': (r'Datos de entrada\PLANILLA GENERAL MARZO 2026.xlsx', 'PLANILA GENERAL MARZO 2026'),
        '2026-04': (r'Datos de entrada\PLANILLA GENERAL ABRIL 2026.xlsx', 'PLANILLA GENERAL ABRIL 2026'),
    }
...
def load_rex_comparisons():
    print("Loading Rex+ payroll comparisons...")
    files = glob.glob(r"c:\Users\Gonzalo Valdivia\Documents\ERP REMU\Detalle proceso *.xlsx")
    if not files:
        print("No Rex+ detail files found matching 'Detalle proceso *.xlsx'")
        return
    ...
    for filepath in sorted(files):
        ...
        idx_ias_anos_servicio = get_col_idx(["indemnizacion de tiempo servido", "indemnizacion tiempo servido", "indemnización de tiempo servido"])
        ...
        for row in sheet.iter_rows(min_row=6, values_only=True):
            ...
            ias_anos_servicio = float(row[idx_ias_anos_servicio]) if idx_ias_anos_servicio is not None and idx_ias_anos_servicio < len(row) and row[idx_ias_anos_servicio] is not None else 0.0
